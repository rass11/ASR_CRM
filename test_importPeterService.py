...
def main(extid):
    response = requests.get(
        "http://apiman.south.rt.ru/apiman-gateway/SZ.BIS/integration-rest/1.0-test/v1/bis4crm-amdocs-rthq/customers/"+extid+"/installmentSummary?authtoken=AAAICQAA5fHQj7Z.6Z1ThmwuoTPQ_PUz&returnCount=true&includeClosed=true"
        , headers=headers)
    return response
...
def test_import_200(extid,result):
    response = main(extid)
    with open("data_PS.txt", "a") as file:
        #r=response.json()
        #r2 = r[1:0]
