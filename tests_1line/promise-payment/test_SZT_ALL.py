...
def main(extid):
    response = requests.get(
        "http://10.42.122.25/apiman-gateway/p1l/accounts-promise-payment/1.0-test/v1/finAccounts/finAccountNumber/"+extid+"/promise-payment?finAccountSysInst=SZT.ALL.BIS&contactId=__&finAccountTZ=%2B03:00"
...
def t2():
    with open('fa_id_SZT_ALL.txt', 'r') as f:
...
def test_import_200(extid,result):
    response = main(extid)
    with open("data_promise-payment_SZT_ALL.txt", "a") as file:
