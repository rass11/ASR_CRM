...
def main(extid,param,reqid):
    ...
    response = requests.get(
        params_config_date2.kurs_adp+extid+"/blockHistory"
...
def test_date_200(extid,param):
    ...
    with io.open('schema_blockHistory_KURS.txt', encoding='utf-8') as data_file:
        schema = json.load(data_file)

    response = main(extid,param,reqid)

    with open("data_blockHistory_KURS_RST_2907_2.txt","a") as file:
        # r=response.json()
        # r2 = r[1:0]
        err_message =''
