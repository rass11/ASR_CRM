...
def main(extid,param):
    response = requests.get(
        params_config_date2.PS_SZ_adp+extid+"/ContentInformation"
...
def t2():
    with open('extid_PS_SZ3.txt', 'r') as f:
...
def test_import_200(extid,result):
    response = main(extid)
    with open("data_payments_PS_SZ2.txt", "a") as file:
        #r=response.json()
        #r2 = r[1:0]
...
def test_date_200(extid,param):
    ...
    with open("data_ContentInformation_PS_SZ_3.txt", "a") as file:
        # r=response.json()
        # r2 = r[1:0]
