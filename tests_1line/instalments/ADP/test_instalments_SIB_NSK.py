...
def main(extid):
    response = requests.get(
        params_config_date.STR_NSK+extid+"/instalments?needClosedInstalments=true",
...
def t2():
    with open('extid_SIB_NSK.txt', 'r') as f:
...
def test_import_200(extid,result):
    response = main(extid)
    with open("data_instalments_SIB_NSK_all.txt", "a") as file:
        # r=response.json()
        # r2 = r[1:0]
