...
def main(extid,param):
    response = requests.get(
        params_config_date2.DSV_PRM_rest+extid+"/penalties"
...
def test_date_200(extid,param):
    with io.open('schema_peni_start.txt', encoding='utf-8') as data_file:
        schema = json.load(data_file)


    response = main(extid,param,)

    with open("data_penalties_START_DSV_PRM_0607.txt","a") as file:
        # r=response.json()
        # r2 = r[1:0]
        err_message =''
