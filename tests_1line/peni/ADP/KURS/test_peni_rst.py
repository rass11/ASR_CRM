...
def main(extid,param):
    response = requests.get(
        params_config_date2.kurs_adp+extid+"/penalties"
...
def t2():
    with open('extid_kurs_RST5.txt', 'r') as f:
...
def test_date_200(extid,param):
    with io.open('schema_peni_kurs.txt', encoding='utf-8') as data_file:
        schema = json.load(data_file)

    response = main(extid, param, )

    with open("data_penalties_kurs_RST_2705.txt", "a") as file:
        # r=response.json()
        # r2 = r[1:0]
        err_message = ''
