...
def main(extid,param):
    response = requests.get(
        params_config_date.start_adp_DSV_PRM+extid+"/accruals"
...
def t2():
    with open('extid_DSV_PRM_STR.txt', 'r') as f:
...
def test_date_200(extid,param):
    with io.open('schema_accruals_start.txt', encoding='utf-8') as data_file:
        schema = json.load(data_file)
    response = main(extid,param,)

    with open("data_accruals_DSV_PRM_0707.txt","a") as file:
        # r=response.json()
        # r2 = r[1:0]
        err_message =''
