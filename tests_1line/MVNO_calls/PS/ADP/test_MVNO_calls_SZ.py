...
def main(extid,param):
    response = requests.get(
        params_config_date2.PS_SZ_adp+extid+"/detalization/mvno-fix"
...
def t2():
    with open('extid_PS_SZ2.txt', 'r') as f:
...
@pytest.mark.parametrize("extid, param",[(ext,param) for ext in t2() for param in params_config_date.params_date2])
def test_date_200(extid,param):
    response = main(extid,param)
    with open("data_MVNO_calls_PS_SZ4.txt", "a") as file:
        # r=response.json()
        # r2 = r[1:0]
