...
def main(extid,param):
    response = requests.get(
        params_config_date2.DSV_PRM_rest+extid+"/infoFinance"
...
def t2():
    with open('extid_DSV_PRM_PSI.txt', 'r') as f:
...
@pytest.mark.parametrize("extid, param",[(ext,param) for ext in t2() for param in params_config_date2.params_date_psi_sib])
def test_date_200(extid,param):
    ...
    with open("data_calculations_START_DSV_PRM_2107_2.txt", "a") as file:
        # r=response.json()
        # r2 = r[1:0]
        err_message = ''
