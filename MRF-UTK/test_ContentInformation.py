...
def main(extid,param):
    xml = '<?xml version="1.0" encoding="utf-8"?><soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/"><soap-env:Body><ns0:GetIPTVCallsRequest xmlns:ns0="http://www.onyma.ru/services/eip/funcs/"><PersonalAccount>'+extid+'</PersonalAccount><DateFrom>'+param["dateStart"]+'</DateFrom><DateTo>'+param["dateStart"]+'</DateTo></ns0:GetIPTVCallsRequest></soap-env:Body></soap-env:Envelope>'

    response = requests.post(
        "http://10.42.122.25/apiman-gateway/MRF-UTK/onyma-soap/1.0-prod"
...
def test_date_200(extid,param):
    ...
    with open(r"./data/STUB_jmeter/MRF_UTK_contentInformation.txt", "a") as file:
        r = response.text
