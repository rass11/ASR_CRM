...
def main(extid,param):
    xml = '<?xml version="1.0" encoding="utf-8"?><soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/"><soap-env:Body><ns0:getCallStatisticsRequest xmlns:ns0="http://south.rt.ru/WsProvider/"><ExternalAccountId>'+extid+'</ExternalAccountId><DateStart>'+param["dateStart"]+'</DateStart><DateEnd>'+param["dateEnd"]+'</DateEnd></ns0:getCallStatisticsRequest></soap-env:Body></soap-env:Envelope>'
...
def test_date_200(extid,param):
    ...
    with open(r"./data/STUB_jmeter/MRF_UTK_calls.txt", "a") as file:
        r = response.text
