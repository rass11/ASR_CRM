...
def main(extid):
    xml = '<?xml version="1.0" encoding="utf-8"?><soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/"><soap-env:Body><ns0:getShortFinanceRequest xmlns:ns0="http://south.rt.ru/WsProvider/"><PersonalAccountID>' + extid + '</PersonalAccountID></ns0:getShortFinanceRequest></soap-env:Body></soap-env:Envelope>'
...
def test_import_200(extid,result):
    response = main(extid)
    with open(r"./data/STUB_jmeter/MRF_UTK_shortFinance.txt", "a") as file:
        #r=response.json()
        #r2 = r[1:0]
        r = response.text
        xml = r.replace('"',r'\"')
